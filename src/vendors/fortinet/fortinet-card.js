...
function buildFortinetToolData(record) {
  return [
    record.cves[0] || "",
...
function buildFortinetTrackerData(record) {
  return [
    record.cves[0] || "",
    record.publishDate || new Date().toISOString().slice(0, 10),
    record.finalSelectedScore != null ? record.finalSelectedScore : "",
...
export function buildFortinetTeamsCardPayload(record) {
    ...
    actions.push({
      ...
      card: {
        ...
        body: [
          {
            type: "TextBlock",
            text: buildFortinetToolData(record),
            wrap: true,
            fontType: "Monospace"
          }
        ]
    ...
    actions.push({
      ...
      card: {
        ...
        body: [
          {
            type: "TextBlock",
            text: buildFortinetTrackerData(record),
            wrap: true,
            fontType: "Monospace"
          }
        ]
    ...
    attachments: [
      {
        ...
        content: {
          ...
          body: [
            ...
            {
              type: "TextBlock",
              text: `**Fortinet CVSS v3.x Score:** ${valueOrFallback(
...
export function buildFortinetDispatchEnvelope(record) {
  return {
    schemaVersion: 1,
    source: "fortinet-advisory-pipeline",
    ...
    teamsPayload: buildFortinetTeamsCardPayload(record)
  };
}
